Shader "Custom/Vertex Displacement/World Space"
{
	Properties
	{
		...
		_Amount("Amount", Range(0.1,100)) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				// Takes the y position of each vert as it was authored and moves it along the object's y-axis
				// The amount it's moved used a sine wave to have a nice wave shape to it
				// The vertex's x position is used otherwise all verts will be moved uniformly
				worldSpaceVertex.y += (-0.99f + sin( _Time.y * _Speed * (worldSpaceVertex.x + worldSpaceVertex.y) * 0.01f * _Amount)) * _Distance;
